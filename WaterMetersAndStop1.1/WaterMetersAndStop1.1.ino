...
{      if (Light == false)
   ...
   } else
 {
  ...
  if (butt1.isTriple()) //запись в EEPROM по тройному клику (нужно сделать например перед принудительной перезагрузкой чтобы не потерять данные)
  {
    lcd.clear();
    lcd.setCursor(0, 0);
    // пишем всё в EEPROM
